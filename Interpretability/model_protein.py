...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dig, pro, dig_mask=None, pro_mask=None):
        ...
        trg_1 = dig
        dig,k = self.sa(dig, dig, dig, dig_mask)
        dig = self.ln(trg_1 + self.do(dig))
        
        trg_2 = dig
        dig, attention = self.ea(dig, pro, pro, pro_mask)
        dig = self.ln(trg_2 + self.do(dig))
        
        trg_3 = dig
        dig = self.ln(trg_3 + self.do(self.pf(dig)))

        return dig,attention
...
class Decoder(nn.Module):
    ...
    def forward(self, dig, pro, dig_mask=None,pro_mask=None):
        ...
        for layer in self.layers:
            dig,attention = layer(dig, pro,dig_mask,pro_mask)
...
class Predictor(nn.Module):
    ...
    def forward(self, compound, adj,  protein,atom_num,protein_num):
        ...
        # enc_dig = [batch size, protein len, hid dim]

        out,attention = self.decoder(compound, enc_dig, compound_mask, protein_mask)
        # out = [batch size, 2]
    ...
    def __call__(self, data, train=True):
        ...
        if train:
            ...
            return predicted_labels,predicted_scores,attention
