...
class Predictor(nn.Module):
    ...
    def forward(self, compound, adj,  protein,atom_num,protein_num):
        ...
        out,norm,dig,sum = self.decoder(compound, enc_dig, compound_mask, protein_mask)

        return out,norm,dig,sum
    ...
    def __call__(self, data, train=True):
        ...
        if train:
            ...
            predicted_interaction,norm,dig,sum = self.forward(compound, adj, protein,atom_num,protein_num)
            correct_labels = correct_interaction.to('cpu').data.numpy()
            ys = F.softmax(predicted_interaction, 1).to('cpu').data.numpy()
            predicted_labels = np.argmax(ys, axis=1)
            predicted_scores = ys[:, 1]
            return predicted_labels,predicted_scores,norm,dig,sum
    ...
    def test(self, dataset, device):
        ...
        with torch.no_grad():
            for data in dataset:
                ...
                predicted_labels,predicted_scores,norm,dig,sum = self.model(data, train=False)
        return predicted_labels,predicted_scores,norm,dig,sum
